...
begin
 ...
 if RTNEdit.Text='' then Abort;
  ...
  if ANS='' then MessageDlg('Routine not exist in UCI !! ' ,mtError, mbOKCancel, 0);
  if ANS='' then Abort;
  MX:=StrToInt(ANS);
  StudioForm.CodeEditor.Clear;
  OpenPBar.Position:=1;
  ...
  for ix:=1 to MX do begin
    ...
    StudioForm.CodeEditor.Append(ANS);
    prb:=ix/MX*100;
  ...
  RTNListBox.Clear;
  StudioForm.HOSTIP.Text:=HOSTIP.Text;
  StudioForm.HOSTPort.Text:=HOSTPort.Text;
  StudioForm.HOSTUCI.Text:=TR(UCIDB,'~','"');
